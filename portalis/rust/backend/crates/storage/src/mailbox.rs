...
pub(crate) const MAILBOX: TableDefinition<&[u8], &[u8]> = TableDefinition::new("mailbox");
/// The next sequence to use for a device, so two deliveries cannot collide.
pub(crate) const MAILBOX_NEXT: TableDefinition<&[u8], u64> = TableDefinition::new("mailbox_next");
...
impl Embedded {
    ...
    pub fn deliver(&self, device: DeviceId, body: &[u8]) -> Result<u64, StorageError> {
        ...
        self.transact(|write| {
            let mut mailbox = write.open_table(MAILBOX)?;
            let (count, bytes) = measure(&mailbox, &device)?;
    ...
    pub fn mailbox_size(&self, device: DeviceId) -> Result<(usize, usize), StorageError> {
        let read = self.begin_read()?;
        let mailbox = read.open_table(MAILBOX)?;
        measure(&mailbox, &device)
    }
    ...
    pub fn drain(&self, device: DeviceId) -> Result<Vec<Item>, StorageError> {
        self.transact(|write| {
            ...
            for row in mailbox.range(low.as_slice()..=high.as_slice())? {
                let (key, value) = row?;
                let sequence = sequence_of(key.value(), device.len())?;
                items.push(Item {
                    sequence,
            ...
            for item in &items {
                mailbox.remove(keyed(&device, item.sequence).as_slice())?;
            }
            Ok(items)
        })
    }
...
/// What a device's mailbox currently costs, in items and bytes.
fn measure(
    mailbox: &impl ReadableTable<&'static [u8], &'static [u8]>,
    device: &[u8],
) -> Result<(usize, usize), StorageError> {
    let (low, high) = prefix_range(device);
    let mut count = 0;
    let mut bytes = 0;
    for row in mailbox.range(low.as_slice()..=high.as_slice())? {
        let (_, value) = row?;
...
/// The sequence a composite key ends with.
fn sequence_of(key: &[u8], prefix: usize) -> Result<u64, StorageError> {
    key.get(prefix..)
...
mod tests {
    ...
    impl Scratch {
        ...
        fn tight(&self) -> Embedded {
            Embedded::with_limits(
                self.0.join("service.redb"),
                Limits {
                    items: 4,
                    bytes: 64,
                },
    ...
    fn a_device_with_nothing_waiting_gets_an_empty_answer() {
        ...
        assert_eq!(store.mailbox_size(MIRA).expect("reads"), (0, 0));
    }
    ...
    fn a_full_mailbox_refuses_rather_than_dropping_silently() {
        ...
        assert_eq!(store.mailbox_size(JONAS).expect("reads"), (2, 64));
        assert!(
            matches!(
    ...
    fn the_item_count_is_bounded_as_well_as_the_size() {
        ...
        assert_eq!(store.mailbox_size(MIRA).expect("reads").0, 4);
        assert!(
            matches!(
    ...
    fn a_key_without_a_sequence_is_reported_as_damage() {
        assert!(matches!(
            sequence_of(b"short", 0),
            Err(StorageError::Malformed)
        ));
        assert_eq!(sequence_of(&keyed(&[1, 2], 7), 2).expect("reads"), 7);
    }
}
