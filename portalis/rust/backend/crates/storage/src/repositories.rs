...
impl UserDirectory for Embedded {
    async fn find_user(&self, user_id: UserId) -> Result<Option<UserRecord>, RepositoryError> {
        Self::find_user(self, user_id).map_err(RepositoryError::from)
    }
    ...
    async fn find_user_by_handle(
        ...
        Self::find_user_by_handle(self, normalized_username, discriminator)
...
impl IdentityRepository for Embedded {
    async fn insert_registration(
        ...
        Self::insert_registration(self, &user, &device).map_err(RepositoryError::from)
    }
    ...
    async fn find_device(
        ...
        Self::find_device(self, device_id).map_err(RepositoryError::from)
    }

    async fn list_devices(&self, user: UserId) -> Result<Vec<DeviceRecord>, RepositoryError> {
        Self::list_devices(self, user).map_err(RepositoryError::from)
    }

    async fn link_device(&self, device: DeviceRecord) -> Result<(), RepositoryError> {
        self.enrol_device(&device).map_err(RepositoryError::from)
    }
...
impl ShareRepository for Embedded {
    async fn find_share(&self, share_id: ShareId) -> Result<Option<ShareRecord>, RepositoryError> {
        Self::find_share(self, share_id).map_err(RepositoryError::from)
    }
    ...
    async fn save_publication(
        ...
        Self::save_publication(self, &share, &snapshot, expected_revision)
            .map_err(RepositoryError::from)
    }
    ...
    async fn find_snapshot(
        ...
        Self::find_snapshot(self, share_id, revision).map_err(RepositoryError::from)
    }
    ...
    async fn revoke_share_access(
        ...
        self.revoke_access(share_id, user_id)
            .map_err(RepositoryError::from)
    }
    ...
    async fn has_share_access(
        ...
        self.has_access(share_id, user_id)
            .map_err(RepositoryError::from)
    }
    ...
    async fn list_authorized_shares(
        ...
        self.shares_readable_by(user_id)
            .map_err(RepositoryError::from)
    }

    async fn list_share_members(&self, share_id: ShareId) -> Result<Vec<UserId>, RepositoryError> {
        self.list_members(share_id).map_err(RepositoryError::from)
    }
}
...
impl Embedded {
    ...
    fn amend_device(
        ...
        let Some(mut device) = Self::find_device(self, device_id)? else {
            return Ok(());
        };
        change(&mut device);
        self.save_device(&device).map_err(RepositoryError::from)
    }
}
